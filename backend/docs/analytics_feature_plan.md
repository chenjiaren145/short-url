...
- **文件**: [store.go](../internal/store/store.go)
- **任务**:
...
- **文件**: [memory.go](../internal/store/memory.go)
- **任务**:
...
- **文件**: [redis.go](../internal/store/redis.go)
- **任务**:
  - 实现 `IncrementVisits` 方法：使用 Redis 的 `INCR` 命令（原子操作），建议 key 格式为 `visits:{shortCode}`。
  - 实现 `GetVisits` 方法：使用 Redis 的 `GET` 命令获取计数。

### 4. 更新业务逻辑层 (Service)
- **文件**: [service.go](../internal/service/service.go)
- **任务**:
...
- **文件**: [handler.go](../internal/api/handler.go)
- **任务**:
...
### 6. 注册路由 (Main)
- **文件**: [main.go](../cmd/short-url/main.go)
- **任务**:
